...
void onFrame() {
	...
	static color LastColor = color();
	if (LastPoint.x != MousePos.x || LastPoint.y != MousePos.y)
	{
		fp x = MousePos.x;
		fp y = MousePos.y;
		color clr = RANDCOLOR;
		fp dx = x - LastPoint.x;
		fp dy = y - LastPoint.y;
		fp dr = clr.r - LastColor.r;
		fp dg = clr.g - LastColor.g;
		fp db = clr.b - LastColor.b;
		fp Count = 100;
		fp stepx = dx / Count;
		fp stepy = dy / Count;
		fp stepr = dr / Count;
		...
		for (int i = 0; i < Count; i++)
		{
			...
			curg += stepg;
			curb += stepb;
			FillCircle(curx - 25, cury - 25, 50, 50, color(curr, curg, curb));
		}
		LastColor = clr;
		LastPoint = MousePos;
	}
