...
void onFrame() {
	...
	{
		//add planet
		Planet newworld = Planet();
		newworld.clr = RANDCOLOR;
		newworld.pos = vec3(RANDFP * width, RANDFP * height, 0);
		newworld.size = RANDFP * 4;
		newworld.ToBeDeleted = false;
		newworld.MassPlus = 0;
		planets[planetcount] = newworld;
		planetcount++;
	}
	const fp GravConst = 0.01;
	//calculate gravity
	for (int i = 0; i < planetcount; i++)
	{
		Planet p1 = planets[i];
		fp p1size3 = p1.size;
		p1size3 = p1size3 * p1size3 * p1size3;
		for (int j = i + 1; j < planetcount; j++)
		{
			Planet p2 = planets[j];
			vec3 d = p2.pos - p1.pos;
			fp ls = d.lengthsquared();
			d = d * (GravConst / ls);
			//add force to p1
			fp p2size = p2.size;
			fp p2size3 = p2size * p2size * p2size;
			p1.speed = p1.speed + d * p2size3;
			//add force to p2
			p2.speed = p2.speed - d * p1size3;
			//colliding?
			fp MinimalDistance = (p1.size + p2.size) * .5;
			if (MinimalDistance * MinimalDistance > ls) 
			{
				//who is the biggest?
				if (p1.size > p2.size) //bug on collisions with >2 planets
				{
					p2.ToBeDeleted = true;//delete p2
					p1.MassPlus = p2size3;
					//add speed
					p1.speed = p1.speed + p2.speed * (p2size3 / p1size3);
				}
				else 
				{
					p1.ToBeDeleted = true;//delete p1
					p2.MassPlus = p1size3;
					//add speed
					p2.speed = p2.speed + p1.speed * (p1size3 / p2size3);
				}
			}
			planets[j] = p2;
		}
		p1.size = pow(p1size3 + p1.MassPlus, 1.0 / 3.0);//3d root
		p1.MassPlus = 0;//clear additional mass
		planets[i] = p1;
	}
	//delete planets
	Planet* ptrOld = planets;
	Planet* ptrNew = planets;
	int NewCount = planetcount;
	for (int i = 0; i < planetcount; i++) 
	{
		if (ptrOld->ToBeDeleted) 
		{
			ptrOld++;
			NewCount--;
		}
	...
	planetcount = NewCount;

	//draw planets to the screen
	ClearColor(color(0));
	for (int i = 0; i < planetcount; i++)
	{
		Planet p = planets[i];
		p.pos = p.pos + p.speed;
		FillCircle(p.pos.x - p.size * .5, p.pos.y - p.size * .5, p.size, p.size, p.clr);
		planets[i] = p;
	}
}
void ProcessInput()
